...
```{r renaming_ir_sa_raw_variables}
...
```{r preview_ir_sa_renamed}
# Look at a preview of the renamed data frame
  # head(ir_sa_renamed)
```

```{r preview_ir_sa_renamed_variable_data_class}
# Note the data class of each variable 
  # glimpse(ir_sa_renamed)
```

```{r convert labelled data to regular factors}
...
```{r preview_ir_sa_cleaned_variable_data_class}
# Review data classes of the newly converted variables 
  # glimpse(ir_sa_clean)
```

```{r set_global_ggplot_theme}
...
```{r boxplot_step_2, warning = FALSE}
...
```{r sankey diagram, warning = FALSE}
library(ggsankey)
